...
if __name__ == '__main__':
    ...
    p_carryig_capacity = sp.stats.uniform.pdf(carrying_capacity, loc=0.01, scale=100)
    ...
    axes.flatten()[3].plot(carrying_capacity, p_carryig_capacity, lw=2, label="$p(MM constant)$")
    axes.flatten()[3].legend(loc='best', fontsize=14)
    axes.flatten()[3].set_xlabel("MM constant")
    ...
    fig, axes = plt.subplots(2,2, figsize=(10, 7))
    axes.flatten()[0].hist(sample_ur,density=True,histtype='step',label='Histogram',color='#A020F0')
    axes.flatten()[0].scatter(sample_ur, sp.stats.gamma.pdf(sample_ur, a=a_r1, loc=0, scale=b_r1), color='#ff7f0e', s=1, label="$p(r_{MR})$")
    axes.flatten()[0].legend(loc='best', fontsize=14)
    axes.flatten()[0].set_xlabel("uptake rate")
    axes.flatten()[1].hist(sample_dr,density=True,histtype='step',label='Histogram',color='#A020F0')
    axes.flatten()[1].scatter(sample_dr, sp.stats.gamma.pdf(sample_dr, a=a_r2, loc=0, scale=b_r2), color='#ff7f0e', s=1, label="$p(r_{D})$")
    axes.flatten()[1].legend(loc='best', fontsize=14)
    axes.flatten()[1].set_xlabel("decay rate")
    axes.flatten()[2].hist(sample_rlr,density=True,histtype='step',label='Histogram',color='#A020F0')
    axes.flatten()[2].scatter(sample_rlr, sp.stats.gamma.pdf(sample_rlr, a=a_r3, loc=0, scale=b_r3), color='#ff7f0e', s=1, label="$p(r_{RL})$")
    axes.flatten()[2].legend(loc='best', fontsize=14)
    ...
    a_sig, b_sig = 10, .05
    sample_iron1 = sp.stats.norm.rvs(size=sampleSize, loc=0.35, scale=0.01)
    sample_iron2 = sp.stats.norm.rvs(size=sampleSize, loc=0.04, scale=0.005)
    sample_iron3 = sp.stats.norm.rvs(size=sampleSize, loc=0.75, scale=0.1)
    sigma2 = sp.stats.gamma.rvs(size=sampleSize, a=a_sig, loc=0, scale=b_sig)
    p_sigma2 = sp.stats.gamma.pdf(sigma2, a=a_sig, loc=0, scale=b_sig)
    p_init1 = sp.stats.norm.pdf(sample_iron1, loc=0.35, scale=0.01)
    p_init2 = sp.stats.norm.pdf(sample_iron2, loc=0.04, scale=0.005)
    p_init3 = sp.stats.norm.pdf(sample_iron3, loc=0.75, scale=0.1)
    fig, axes = plt.subplots(2,2, figsize=(10, 7))
    axes.flatten()[0].hist(sample_ur,density=True,histtype='step',label='Histogram',color='#A020F0')
    axes.flatten()[0].scatter(sample_ur, sp.stats.gamma.pdf(sample_ur, a=a_r1, loc=0, scale=b_r1), color='#ff7f0e', s=1, label="$p(sigma^2)$")
    axes.flatten()[0].legend(loc='best', fontsize=14)
    axes.flatten()[0].set_xlabel("$\sigma^2$")
    axes.flatten()[1].hist(sample_iron3,density=True,histtype='step',label='Histogram',color='#A020F0')
    axes.flatten()[1].scatter(sample_iron3, p_init3, color='#ff7f0e', s=1, label="$p(Fe_{M}(t=0))$")
    axes.flatten()[1].legend(loc='best', fontsize=14)
    axes.flatten()[1].set_xlabel("Fe(t=0) in matrix")
    axes.flatten()[2].hist(sample_iron1,density=True,histtype='step',label='Histogram',color='#A020F0')
    axes.flatten()[2].scatter(sample_iron1, p_init1, color='#ff7f0e', s=1, label="$p(Fe_{R}(t=0))$")
    axes.flatten()[2].legend(loc='best', fontsize=14)
    axes.flatten()[2].set_xlabel("Fe(t=0) in roots")
    axes.flatten()[3].hist(sample_iron2,density=True,histtype='step',label='Histogram',color='#A020F0')
    axes.flatten()[3].scatter(sample_iron2, p_init2, color='#ff7f0e', s=1, label="$p(Fe_{L}(t=0))$")
    axes.flatten()[3].legend(loc='best', fontsize=14)
    axes.flatten()[3].set_xlabel("Fe(t=0) in leaves")
    fig.tight_layout(pad=1.0)
    plt.savefig('priors_initcond.png')
    # # save sample
