...
#     r_dec_l = rate_decay_l(t)  # increase in weight of leafs in g
#     # matrix_weight = 10 #matrix weight in gramms to bring everything to the same measurement units
#     dxdt = [(r_mr*dr_w*fe_in_m)/(fe_in_m + MMc) - r_rl * fe_root - r_dec_r * fe_root, \
#             r_rl * fe_root - r_dec_l * fe_leaf]
#     return dxdt
...
#     return -np.inf
#
# # Model 2 - mic concentration & no decay rate
# def log_prior(theta):
#     # prior without init conditions
#     sigma2, r_mr, r_rl, c_min, MMc, *iron_0  = theta
#     # everything that is defined in the conditions after the first one corresponds to uniform priors
#     if all(c >= 0 for c in theta) and (0.01 < MMc < 100) and (0.001 < c_min < 0.6):
#         a_r1, b_r1 = 10, .06
#         a_r3, b_r3 = 10, .05
#         a_sig, b_sig = 10, .05
#         p_uptake_r = sp.stats.gamma.pdf(r_mr, a=a_r1, loc=0, scale=b_r1)
...
#         p_sigma2 = sp.stats.gamma.pdf(sigma2, a=a_sig, loc=0, scale=b_sig)
#         p_init = sp.stats.multivariate_normal.pdf(iron_0, mean=iron_0_true, cov=[[0.2, 0], [0, 0.1]])
#         lprior = np.log(p_uptake_r*p_r2l_r*p_sigma2*p_init)
#         return lprior
#     # returns a scalart
#     return -np.inf
#
# # Model 2 - mic concentration & no decay rate & no c_min
# def log_prior(theta):
#     # prior without init conditions
#     sigma2, r_mr, r_rl, c_min, MMc, *iron_0  = theta
#     # everything that is defined in the conditions after the first one corresponds to uniform priors
#     if all(c >= 0 for c in theta) and (0 < MMc < 10):
#         a_r1, b_r1 = 10, .06
#         a_r3, b_r3 = 10, .05
#         a_sig, b_sig = 10, .05
#         p_uptake_r = sp.stats.gamma.pdf(r_mr, a=a_r1, loc=0, scale=b_r1)
...
# def log_prior(theta):
#     sigma2, r_mr, r_rl, c_max, *iron_0  = theta
#     # everything that is defined in the conditions after the first one corresponds to uniform priors
#     if all(c >= 0 for c in theta) and (0 < c_max < 10):
#         a_r1, b_r1 = 10, .06
#         a_r2, b_r2 = 5, .01
#         a_r3, b_r3 = 10, .05
#         a_sig, b_sig = 10, .05
...
def log_likelihood(theta, times):
    ...
    *ode_params, iron_r, iron_l = model_params
    for iExperiment in range(nExperiments):
        #  make the time varying functions global and easy to access in the models
        global dry_weight, matrix_content, rate_decay_r, rate_decay_l
        dry_weight = dw_all[iExperiment]
        matrix_content = x_matrix_all[iExperiment]
        rate_decay_r = rate_roots[iExperiment]
        rate_decay_l = rate_leaves[iExperiment]
        y_exp = y_all[iExperiment]
...
if __name__ == '__main__':
    ...
    for iExperiment in Experiments:
        ...
        for _, Pt in enumerate(Parts):
            ...
            iron_0_true.append(init_val_kg / 1000) # convert everything to be per gram
        ...
        for _, Rep in enumerate(Replicates):
            ...
            print(y_fe[Rep])
        y_all.append(y_fe)
        inits.append(iron_0_true)
        # create an interpolator for the dry weight and iron content in the matrix
        dw_all.append(sp.interpolate.interp1d(SamplingDays, df_dw_r.iloc[:,iExperiment].values, fill_value='extrapolate'))
        x_matrix_all.append(sp.interpolate.interp1d(SamplingDays, df_matrix.iloc[:,iExperiment].values,  fill_value='extrapolate'))
        rate_roots.append(sp.interpolate.interp1d(SamplingDays[1:], df_dw_r_diff.iloc[1:, iExperiment].values,
                                               kind='cubic',
                                               fill_value=(
                                                   df_dw_r_diff.iloc[1, iExperiment],
                                                   df_dw_r_diff.iloc[-1, iExperiment]),
                                               bounds_error=False))
        rate_leaves.append(sp.interpolate.interp1d(SamplingDays[1:], df_dw_l_diff.iloc[1:, iExperiment].values,
                                               fill_value=(
                                                   df_dw_l_diff.iloc[1, iExperiment],
                                                   df_dw_l_diff.iloc[-1, iExperiment]), \
    ...
    sigma_vector = np.array([0.0001, 0.0000001, 0.0000001, .01, 0.001])
