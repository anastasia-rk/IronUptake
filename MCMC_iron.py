...
def log_prior(theta):
    ...
    if all(c >= 0 for c in theta) and (100 < c_max < 10000) and (r_mr < 100):
        a_r1, b_r1 = 10, .6
        a_r2, b_r2 = 5, .1
        a_r3, b_r3 = 10, .5
        a_sig, b_sig = 10, .5
...
def log_likelihood(theta, times):
    ...
    model_output = odeint(ode_iron_leaf_root, iron_0, times, args=(model_params,))
...
def ode_iron_leaf_root(x, t, params):
    ...
    matrix_weight = 30 #matrix weight in gramms to bring everything to the same measurement units
...
if __name__ == '__main__':
    ...
    # test the model: sigma2, rates
    sig2 = 1
    params_0 = [sig2] + [.1, .1, .1, 500]
    covar = [[0.1, 0, 0, 0, 0], [0, 0.01, 0, 0, 0], [0, 0, 0.01, 0, 0], [0, 0, 0, 0.01, 0], [0, 0, 0, 0, 500]]
